#include "FSM/FSMState_AutoTakeout1.hpp"

using namespace std::chrono_literals;

/* 创建状态类 */
FSMState_AutoTakeout1::FSMState_AutoTakeout1():
                  FSMState(FSMStateName::AUTO_TAKEOUT_1, "auto_takeout_1"){}

/* 创建 rosbag 读取指针 */
// rosbag2_storage::StorageOptions storage_options(
//     {"/home/engineer/engineer_ws/src/Engineer_ROS2_controller/engineer_bag/auto_takeout_1_1/auto_takeout_1_1_0.db3",
//      "sqlite3"});
...
void FSMState_AutoTakeout1::enter()
{
    /* 初始化 rosbag 读取指针 */
    rosbag2_storage::StorageOptions storage_options(
        {"/home/engineer/engineer_ws/src/Engineer_ROS2_controller/engineer_bag/auto_takeout_1_1/auto_takeout_1_1_0.db3",
...
void FSMState_AutoTakeout1::run()
{
    engineer_msg::msg::LiftState lift_state;
    engineer_msg::msg::SucketState sucket_state;

    if ((!lift_ready) && (lift_position_real < 95))
    {
        lift_state.rise = true;
        lift_state.fall = false;
    }
    else if ((!lift_ready) && (lift_position_real > 120))
    {
        lift_state.rise = false;
        lift_state.fall = true;
    }
    else if ((this->BagPlay_Counter >= 500) && (lift_position_real < 250))
    {
        lift_state.rise = true;
        lift_state.fall = false;
    }
    ...
    sucket_target = true;
    sucket_state.state = true;
    sucket_command_publisher->publish(sucket_state);
}

void FSMState_AutoTakeout1::exit()
{
    /* NULL */
}
